...
static int ns_use_cert(SSL_CTX *ctx, const char *pem_file) {
	if (ctx == NULL) {
		...
		//int ret = SSL_CTX_use_certificate_file(ctx, pem_file, 1); //"E:\\tmp\\CMAKE_BUILDS\\ssl_wrapper\\Debug\\cert.pem", 1); //pem_file, 1);
		//int ret1 = SSL_CTX_use_PrivateKey_file(ctx, pem_file, 1); //"E:\\tmp\\CMAKE_BUILDS\\ssl_wrapper\\Debug\\cert.pem", 1); //pem_file, 1);
		...
		bio = BIO_new_file(pem_file, "r"); //"E:\\tmp\\CMAKE_BUILDS\\ssl_wrapper\\Debug\\dhparams.pem", "r"); // pem_file, "r");
		if (bio != NULL) {
		...
		if(SSL_CTX_use_certificate_chain_file(ctx, pem_file) == 1) return 0;
		return -2;
	}
}
#endif  // NS_ENABLE_SSL
